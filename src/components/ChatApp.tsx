...
interface UserProfile {
  ...
  status?: 'pending' | 'requested' | 'accepted' | 'blocked';
  requestedBy?: string;
  relation?: 'pending' | 'requested' | 'accepted' | 'blocked' | null;
...
export default function ChatApp() {
  ...
  useEffect(() => {
    ...
    // Listen for inbound pending requests only. The sender's local row uses
    // status='requested', while the recipient's row uses status='pending'.
    // Also verify requestedBy so malformed legacy rows cannot show you as an
    ...
    const unsubscribePending = onSnapshot(pendingQuery, async (snapshot) => {
      const incomingDocs = snapshot.docs.filter((d) => {
        ...
        return requestData.userId === user.id &&
          requestData.requestedBy &&
          requestData.requestedBy !== user.id &&
          requestData.friendId === requestData.requestedBy;
      });
      const pendingData = await Promise.all(incomingDocs.map(async (d) => {
        const friendData = d.data();
        const requesterId = friendData.requestedBy;
  ...
  const sendFriendRequest = async (friendId: string) => {
    ...
    try {
      ...
      if (existingData?.status === 'requested') {
        setToast({ message: "Friend request is already pending.", type: 'info' });
        return;
      }
      ...
      // Store mirrored relationship docs atomically. The sender's row is
      // deliberately 'requested' instead of 'pending', so the sender's pending
      // invite listener can never show their own outgoing request as an invite.
      ...
      setSearchResults(prev => prev.map(res =>
        res.id === friendId ? { ...res, relation: 'requested', relationRequestedBy: user.id } : res
      ));
    } catch (err: any) {
                  ...
                  <AnimatePresence mode="popLayout">
                    {searchResults.map((result) => (
                      ...
                      >
                        ...
                        ) : (result.relation === 'pending' || result.relation === 'requested') && result.relationRequestedBy === user?.id ? (
